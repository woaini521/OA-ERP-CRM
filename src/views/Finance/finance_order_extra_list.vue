    ...
    <div class="box">
        ...
        <div class="content_box">
            <el-table :data="tabledata">
                ...
                <el-table-column label="原因" prop="remarks"></el-table-column> 
        ...
        <el-dialog title="财务审核" :visible.sync="examine">
            <p style="font-size: 20px;color:#000">
                <span>订单编号：</span><span>{{ receiving_name }}</span>
                <span style="margin-left:30px;">原因：</span><span>{{ receiving_account }}</span>
            </p>
            <p style="font-size: 20px;color:#000;margin-top:5px;"><span>金额：</span><span>{{ fee }}￥</span></p>
            ...
            <div class="contract">
                ...
                <div class="contractInner">
                    ...
                    <el-upload
                        ...
                        action="/file/customer_extra_charge_file"
    ...
    methods:{
        ...
        seachName(){
            this.axios.post('/Finance/finance_order_extra_list',{
                ...
                this.currentPage = res.data.extra.current_page;
                this.total = res.data.extra.total;
                this.per_page = res.data.extra.per_page;
                this.last_page = res.data.extra.last_page;
            })
        },
        // 获取列表数据
        gettabledata(){
            this.axios.get('/Finance/finance_order_extra_list').then(res => {
                this.tabledata = res.data.extra.data;
                this.currentPage = res.data.extra.current_page;
                this.total = res.data.extra.total;
                this.per_page = res.data.extra.per_page;
        ...
        FinanceSee(a){
            ...
            this.receiving_account = a.remarks;
            this.fee = a.money; 
        },
        // 财务审核提交
        Submission(){
            this.axios.post('/Finance/finance_order_extra_status',{
                id:this.id,
            }).then(res => {
        ...
        // 获取合同图片
        getimgUrl(a){
            this.axios.get('/crm.Order/customer_order_file_select?id='+a+'&data_table=customer_order_extra&correlation_id='+this.receiving_name).then(res => {
                if(res.data != null){
                 this.imageUrlstate = true;
                 this.imageUrl = res.data.customer_order_extra;
                }  
            })
        },
        // 删除合同数据图片
