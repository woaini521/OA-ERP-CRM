      ...
      <div class="head_box">
       <el-select
        ...
        <div class="danxuan">
          ...
          <div class="danxuan2">
            <h4>发票信息</h4>
            <el-radio v-model="piao" v-for="(item,index) in InvoiceType[Number(radio)-1]" :key="index" :label="item.id">{{item.name}}{{item.type}}{{item.percentile}}</el-radio> 
          </div>
        </div>  
    ...
    methods:{
      ...
      addOrder(){
        if(this.customer_order_id == ''){
          if(this.endtime=='' || this.state4 == '' || this.starttime == '' || this.kehuValue == ''|| this.piao == ''){
      ...
      PostaddOrder(){
        ...
        this.axios.post('/crm.Order/order_add',{
      ...
      // 获取合同图片
      getimgUrl(a){
        this.axios.get('/crm.Order/customer_order_file_select?id='+a+'&class=customer_order').then(res => {
          if(res.data != null){
