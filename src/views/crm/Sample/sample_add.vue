...
import excel from "@/components/excel";
  ...
  export default {
    ...
    methods:{
      // 生成订单客户信息
      addOrder(){
        console.log(this.customer_order_id)
      ...
      tijiao(){
        if(this.formAddress.id == ''){
            if(this.formAddress.address == '' || this.formAddress.name == '' || this.formAddress.phone == '' || this.formAddress.sheng == '' || this.formAddress.shi == '' || this.formAddress.qu == '' || this.kehuValue == '' ){
      ...
      // 订单草稿箱
      getcaogao(){
        this.axios.get('/crm.Sample/sample_update?id=0').then(res => {
