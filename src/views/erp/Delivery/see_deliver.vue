...
<template>
  <div class="box">  
    <div class="head_box">
        <el-select v-model="state4" filterable remote reserve-keyword placeholder="请输入公司关键词" :remote-method="remoteMethod"
           disabled @change="kehuName">
        ...
        <div class="riqi">
          <div class="xiaohou">
            <span>销售日期</span> <el-date-picker v-model="starttime" disabled type="date" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="margin-left:20px;"></el-date-picker>
          </div>
          <div class="fahuo">
            <span>收货日期</span> <el-date-picker v-model="endtime" disabled type="date" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="margin-left:20px;"></el-date-picker>
          </div>
        </div>
        <div class="danxuan">
          <div class="danxuan1">
        ...
        <hr>
          <div class="generatedAddressInner" v-for="item in fixedAddress" :key="item.id">
            <div class="generatedAddressInnerLeft">
              ...
              <deliver :shuju="item.purchase_product" :options="express" :tableDataWuliu="item.order_express" :tableDataFeiyong="item.order_express_cost" @func="getMsgFormSon" xian='1'></deliver>
            </div> 
          </div>       
      </div>
...
import deliver from "@/views/erp/Delivery/deliver";
  export default {
    ...
    getMsgFormSon(data){
        if(data == 1){
            this.axios.get('/erp.Delivery/delivery_order_find?customer_order_id='+this.$route.params.id).then(res => {
                    // 配货信息
              this.fixedAddress = res.data.address
        ...
        this.axios.get('/erp.Delivery/delivery_order_find?customer_order_id='+this.$route.params.id).then(res => {
          if(res.data != null){
            this.$nextTick(function (){
                // 头部信息
              this.kehuName(res.data.order.company_id);
              this.state4 = res.data.order.company_name;
              this.kehuValue = res.data.order.customer_id;
              this.starttime = res.data.order.sales_time;
              this.endtime = res.data.order.delivery_time;
              this.class_id = String(res.data.order.class_id);
              this.radio = String(res.data.order.type_id);
              this.remarks = res.data.order.remarks;
              this.repair_invoice =res.data.order.repair_invoice;
              this.repair_freight =res.data.order.repair_freight;
              this.totalprice= res.data.order.total_price;
              this.totalcostprice= res.data.order.total_cost_price; 
              this.userprice= res.data.order.user_price; 
              this.commissionprice = res.data.order.commission_price; 
              this.logisticsprice = res.data.order.logistics_price; 
              this.extraprice = res.data.order.extra_price;
              this.invoiceprice = res.data.order.invoice_price; 
              this.aftersalesprice = res.data.order.after_sales_price;
              this.invoice_name =  res.data.order.invoice_name;
              this.invoice_tax =  res.data.order.invoice_tax;
              this.invoice_type =  res.data.order.invoice_type;
               // 产品信息
              this.tableData = res.data.product_sku;
