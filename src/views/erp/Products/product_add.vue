            ...
            <el-table :data="SKUdata" style="width: 100%">
                ...
                <el-table-column prop="firm_id" label="是否是专版">
                    <template slot-scope="scope">
                        <span>{{scope.row.firm_id == 1 ? '否' : '是'}}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="logo" label="定制logo">
                    <template slot-scope="scope">
                        <span>{{scope.row.logo == 1 ? '否' : '是'}}</span>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods:{
           ...
           // this.queryimg();
            if(this.formProduct.name == '' || this.formProduct.category == ''|| this.formProduct.type == ''){
                ...
                if(this.Product.product_id == ''){
                    this.axios.post('/erp.Product/product_add',{
        ...
        addSKU(){
            if(this.Product.id == ''){
                this.axios.post('/erp.Product/product_sku_add',{
                status:'',
                rank:'',
        ...
        // 获取原有的SKU
        getSKUdata(){
            this.axios.post('/erp.Product/product_sku_list',{
                product_id:this.Product.product_id
            }).then(res => {
        ...
        edit(a){
            ...
            this.Product.product_id = a.product_id;
            ...
            this.Product.cost = Number(a.cost_price);
            this.Product.Sell = Number(a.selling_price);
        ...
        addSupplierPrice(){
            if(this.formProduct.name == '' || this.formProduct.category == ''){
                ...
                this.axios.get('/erp.Product/product_update?id='+this.Product.product_id).then(res => {
                    this.optionproduct = res.data.sku;
                })    
        ...
        SupplierPriceModify(a){
            ...
            this.SteppedPrice.product = String(a.id);
        },
        SupplierPriceEdit(a){
        ...
        comfirm(){
            if(this.Product.product_id == ''){
               this.open('先添加SKU','error'); 
            }else if(this.SupplierPrice.length == 0){
                this.open('先添加供应商阶梯价','error');
            }else{
